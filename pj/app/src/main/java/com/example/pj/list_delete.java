...
public class list_delete extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                sqlDB.execSQL("DELETE FROM readTBL WHERE title = '"
                        + title + "';");
        ...
        btnSelect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                sqlDB = Helper.getReadableDatabase(); // 읽기 가능한 데이터베이스
                ...
                //read 테이블의 데이터 조회
                Cursor cursor = sqlDB.rawQuery("select * from readTBL where title = ?", new String[] { title });

                cursor.moveToNext();
                int total = cursor.getCount();
                String c_total = String.valueOf(total);

                //조회한 책이 read테이블에 없을 시 토스트 메시지 띄우기
                if(total==0){
                    //조회한 책이 read 테이블에 없을 경우 안보이게 함
                    ivCover.setVisibility(View.INVISIBLE);
                    etTitle.setVisibility(View.INVISIBLE);
                    etAuthor.setVisibility(View.INVISIBLE);
                    etPublisher.setVisibility(View.INVISIBLE);
